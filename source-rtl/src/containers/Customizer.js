...
class Customizer extends Component {
  ...
  handleColorChange = (varname, color) => {
    ...
    window.less
      .modifyVars(vars)
      .then(() => {
        message.success(`با موفقیت به روز شد`);
        this.setState({ vars });
        localStorage.setItem("app-theme", JSON.stringify(vars));
      })
      .catch(error => {
        message.error(`به روزرسانی انجام نشد`);
      });
  };
  ...
  resetTheme = () => {
    ...
    window.less
      ...
      .catch(error => {
        message.error(`تنظیم مجدد طرح انجام نشد`);
      });
  };
  toggleCustomizer = () => {
  ...
  getCustomizerContent = () => {
    ...
    return <CustomScrollbars className="gx-customizer">
      <div className="gx-customizer-item">
        <h6 className="gx-mb-3 gx-text-uppercase">قالب</h6>
        <Radio.Group value={themeType} onChange={this.onThemeTypeChange}>
          <Radio.Button value={THEME_TYPE_LITE}>روشن</Radio.Button>
          <Radio.Button value={THEME_TYPE_SEMI_DARK}>نیمه روشن</Radio.Button>
          <Radio.Button value={THEME_TYPE_DARK}>تیره</Radio.Button>
        </Radio.Group>
      </div>
      <div className="gx-customizer-item">
        <h6 className="gx-mb-3 gx-text-uppercase">رنگ ها</h6>
        <Radio.Group className="gx-mb-3" value={colorSelection} onChange={this.onColorSelectionTypeChange}>
          <Radio.Button value={THEME_COLOR_SELECTION_PRESET}>پالت های از پیش تعیین شده </Radio.Button>
          <Radio.Button value={THEME_COLOR_SELECTION_CUSTOMIZE}>شخصی سازی</Radio.Button>
        </Radio.Group>
      ...
      <h6 className="gx-mb-3 gx-text-uppercase">استایل منو</h6>

      {this.getNavStyles(navStyle)}

      <h6 className="gx-mb-3 gx-text-uppercase">صفحات</h6>
